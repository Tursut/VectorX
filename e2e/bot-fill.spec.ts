...
test('1 human + 3 bots: lobby shows 3 bot slots, game starts, board renders', async ({ page }) => {
  ...
  const firstValid = page.locator('[data-testid="game-board"] [role="button"]').first();
  ...
  const fills = await page.locator('.cell-fill').count();
