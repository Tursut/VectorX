...
- Current seat is a bot → `setAlarm(Date.now() + computeTurnDelay(game, lobby))`.
- Current seat is a human → `setAlarm(Date.now() + computeTurnDelay(game, lobby))` (normally `TURN_TIME_MS`, plus bumps on turn 0 / roulette / trap when applicable).
...
**Browser / OS “back”** — `src/useBackGuard.js` pushes a sentinel for local play + online gameplay/lobby. StartScreen drawers use a sibling `useLayoutEffect` sentinel (`vxStartDrawer`) + `popstate` peel (no re-arm). Entering “GOT A CODE?” join mode pushes a second entry so the first back returns to the HOST/join toggle (create room) view before the sentinel peels to the hero menu. Lobby confirm exit lands in the friends drawer (`onExit({ target: 'friends' })`); quitting mid-online-play lands on the hero menu (`target: 'menu'`). Local quick-play countdown + in-progress play share the exit overlay.
